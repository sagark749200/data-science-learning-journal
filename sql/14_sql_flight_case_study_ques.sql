-- connect xampp and mySQL and then percenile_cont, Dist will work 
-- bcz xampp work on MariaDB and that allows the percentile func

--------------------------------------
-- SQL DATETIME CASE STUDY QUESTIONS--
--------------------------------------
...
-- new col (helpful almost in all the questions)
ALTER TABLE flights
ADD COLUMN duration_mins INTEGER,
ADD COLUMN arrival DATETIME;

-- change time into mins
SELECT Duration,
...
-- update 5m --> 4h 5m (outlier)
UPDATE flights
SET duration = '4h 5m'
WHERE duration = '5m';

-- now update mins as total duration
UPDATE flights
...
SELECT * FROM flights;
-- final answer
SELECT `index`, time(departure), TIME(arrival) FROM flights;



-- 	7. Calculate the arrival date for all the flights
SELECT `index`, date(departure), date(arrival) FROM flights;
...
-- (hard version) Calculate the average duration of flights between all city pairs. The answer should In xh ym format
...
-- 12. find the longest flight distance(btw cities in terms of time)
SELECT source,destination,AVG(duration_mins) AS 'avg_duration' FROM flights
GROUP BY source,destination
...
-- also try chart with google sheets
